  ...
  <div>
    ...
    <div class="card settings-section" style="margin-top:1.5rem">
      ...
      <div class="add-row">
        ...
        <select v-model="newUser.role" class="form-input" style="max-width:150px">
          <option value="admin">مدير كامل</option>
          <option value="scanner">ماسح فقط</option>
        </select>
           ...
           <tr v-for="u in users" :key="u.id" style="border-bottom:1px solid rgba(255,255,255,0.05)">
             ...
             <td style="padding:0.8rem 0.5rem">
                <span class="badge" :class="u.role === 'admin' ? 'badge-primary' : 'badge-secondary'" style="font-size:0.75rem">
                  {{ u.role === 'admin' ? 'مدير النظام' : 'تطبيق ماسح QRCode' }}
                </span>
             </td>
             <td style="padding:0.8rem 0.5rem">
...
async function addDept() {
  ...
  try {
    await createDepartment({ name: newDept.value.trim() })
    newDept.value = ''
